...
export async function projectMetadataForInputFile(
  ...
  if (project.dir && project.config) {
    ...
    // if (project.memoryCache[memoryCacheKey]) {
    //   return ld.cloneDeep(project.memoryCache[memoryCacheKey] as Metadata);
    // }
    ...
    // total_1 += t2 - t1;
    // total_2 += t3 - t2;
    console.log(memoryCacheKey.length, JSON.stringify(result).length);
    return result;
  } else {
